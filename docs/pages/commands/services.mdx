...
Jack Cloud invokes crons by sending `POST /__scheduled` to your worker's HTTP handler. You must add an explicit route for this. Cloudflare's native `scheduled()` export does **not** work with Jack Cloud — Jack uses HTTP, not the Workers scheduled event.

:::warning
**`await` your work before returning the response.** Do not use `waitUntil` for cron work — background tasks started with `c.executionCtx.waitUntil()` may be killed before they complete. The cron handler should finish all work before sending the response.
:::
...
app.post('/__scheduled', async (c) => {
  ...
  await doWork(c.env);  // finish work before responding
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && url.pathname === '/__scheduled') {
      await doWork(env);  // finish work before responding
...
1. Creates the resource in Jack Cloud (managed mode) or your Cloudflare account (BYO mode)
