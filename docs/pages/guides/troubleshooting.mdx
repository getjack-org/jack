...
Your session expired or was never set up.
...
# Headless / agent (email verification, no browser)
jack login --email you@example.com
# then: jack login --email you@example.com --code <6-digit-code>
...
You ran `jack login` in a non-interactive environment (CI, Docker, SSH, agent) without specifying an email.

**Fix:** Use the headless login flow:

```bash
jack login --email you@example.com
```

Or set `JACK_API_TOKEN` if you already have a token.
